...
export function TransactionsPage() {
  ...
  const load = useCallback(async () => {
    setError(null);
    const res = await apiJson<ListResponse>("/transactions?limit=100&offset=0");
    setData(res);
  }, []);
        ...
        {!loading && data ? (
          <>
            ...
            {data.transactions.length === 0 ? (
              <p className="muted">No ledger rows yet. Complete an import session and run import from the home page.</p>
            ) : (
