...
export async function upsertGcalEvent(
  ...
  input: Omit<CreateFamilyEventInput, "recordType" | "source" | "gcalEventId" | "gcalCalendarId">
): Promise<FamilyEvent> {
  const existing = await qGet<FamilyEventRow>(
    `SELECT * FROM family_events WHERE gcal_event_id = ? AND household_id = ?`,
    gcalEventId,
    householdId
  );
    ...
    const row = await qGet<FamilyEventRow>(
      `UPDATE family_events
       SET title = ?, description = ?, start_at = ?, end_at = ?, location = ?,
           all_day = ?, is_active = TRUE, updated_at = NOW()
       WHERE gcal_event_id = ? AND household_id = ?
       RETURNING *`,
      input.title,
      input.description ?? null,
      input.startAt ?? null,
  ...
  return createFamilyEvent(householdId, {
    ...input,
    recordType: "event",
    source: "gcal",
    gcalEventId,
