...
export function renderPasswordResetTemplate({ resetLink }: PasswordResetTemplateInput): EmailTemplate {
  ...
  const html = hasResetLink
    ? layout({
      title: "Reset your Grove password",
      content: `
    ...
    : layout({
      ...
      content: `
        ...
        <p style="margin:0 0 8px;color:#4b5563;line-height:1.6;">
          To reset your password, open the Grove app and go to
  ...
  const text = hasResetLink
    ...
    : [
      ...
      "",
      "We received a request to reset your Grove password.",
      "To reset your password, open the Grove app and go to Sign In -> Forgot password, then enter the token below:",
      resetLink,
      "",
      "This token expires in 1 hour. If you didn't request this, ignore this email."
    ].join("\n");
