...
| **Classification (Epic 5.1)** | 🟡 | **Static rules** in **`category-rules.ts`** + **DB rules** (migration **`0009`**, **`category_rule`** table) evaluated before defaults; **`classification_meta`** on canonical rows for explainability. **`GET/POST/PATCH /categories/rules`**; **UI:** **`/categories/rules`** (built-in rules grouped by category + amount scope with priority range in group header). **Default taxonomy** names/ids: **`0001_seed_defaults`** + migrations through **`0029`** (display-name sync); see **`docs/CATEGORIZATION_ROADMAP.md`**. **`unknown_category`** triage: **Transactions → Needs review** (bulk + expand-row context + **`POST /resolution/bulk-apply-category`**). **Still not:** richer confidence UX polish |
