...
| **PT-17** | AI oral ARB script generation | Feature | **Deferred** | When `protest_worksheet.status = 'arb'` (formal hearing scheduled), new AI tool call `generate_arb_script` produces step-by-step oral presentation: opening statement, attack on DCAD's sales comps (using comp annotation notes), §41.43 argument using DCAD's own equity median, taxpayer's supporting comps, neighborhood comparison summary, closing ask, per-step IF/THEN appraiser rebuttals, AI-derived negotiation thresholds. Prerequisite: PT-9 (comp annotations) must be complete. GH #66. |
