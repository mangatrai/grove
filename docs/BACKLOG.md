...
| **FR-15 v2** | Household staff payroll (timesheets, expenses, payroll, taxes) | **V7 committed** | Full payroll for household employees (nanny, cleaner, au pair): variable-rate timesheets w/ FLSA OT, reimbursements vs taxable pay items, gross-to-net (FICA, optional W-4 FIT via Pub 15-T, FUTA, TX SUTA), compliance calendar (1040-ES, TWC, W-2, Schedule H, TX new-hire), PDF pay stubs, filled Schedule H PDF + W-2 box exports. `staff` RBAC role + portal. No code yet. Correct-to-the-cent bar. Epic GH #121 + PY-1…PY-9. See PRD_AND_CRS.md §3.12. | Schema (designed): `staff_profile`, `staff_rate`, `timesheet_entry`, `timesheet_period`, `staff_expense`, `staff_pay_item`, `payroll_run`, `payroll_run_line`, `staff_payment`, `tax_remittance`, `tax_year_config`. All in `EXPORT_REGISTRY`. |
