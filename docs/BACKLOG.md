...
| **PT-7** | Protest status — branching flow + outcome tracking | Feature | **Active** | Replace linear stepper with branching status flow matching real protest resolution paths. Add `outcome` (settled_informal/won_arb/lost_arb/withdrawn) and `informal_offer_usd` to worksheet. Contextual action buttons per status stage instead of dropdown. Resolved state shows outcome badge + savings summary. Migration 0058. GH #51. |
