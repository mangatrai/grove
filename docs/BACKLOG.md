...
| **PT-12** | Chat context summarization — prevent `conversation_json` unbounded growth | Feature | **Backlog** | `conversation_json` grows with every turn and will eventually exceed OpenAI context limits. Strategy: after N turns (e.g., 20), summarize older turns into a single system message and truncate the raw array. Triggered on POST /chat before calling OpenAI. GH #62. |
...
| **OPS-1** | GDrive backup scheduler → node-cron nightly 11 PM CT | Ops | **Backlog** | Replace 30-min `setInterval` heartbeat with `node-cron` at `0 23 * * *` (America/Chicago). Per-household `backup_frequency_hours` elapsed check stays — scheduling becomes predictable instead of drifting from server boot time. GH #68. |
| **OPS-2** | Realty valuation scheduler → node-cron 1st of month 10 PM CT | Ops | **Backlog** | Replace 6-hour `setInterval` with `node-cron` at `0 22 1 * *` (America/Chicago). 28-day staleness query unchanged. Reduces unnecessary API calls from 4×/day to once/month. GH #69. |
| **OPS-3** | Export cleanup scheduler → node-cron hourly | Ops | **Backlog** | Replace `setInterval(1h)` in `export-job.service.ts` with `node-cron` at `0 * * * *` (America/Chicago). Same TTL behavior, consistent with other node-cron schedulers. GH #70. |
| **OPS-4** | Nightly cleanup of staged import files on disk after 30 days | Ops | **Backlog** | Nightly cron (1 AM CT) queries `import_file.stored_path` where `import_session.started_at < NOW() - 30 days`, `fs.unlinkSync` each file, sets `stored_path = NULL`. Covers all statuses (`created`, `processing`, `review`, `finalized`, `failed`). DB rows kept forever — audit trail + rollback. GH #71. |
