...
2. A **worker** (same Node process with a poller, or a separate worker) runs canonicalize.
...
Implementation can follow the same session locking and progress ideas as other long-running import steps.
