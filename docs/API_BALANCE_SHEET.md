...
- **`assets`** / **`liabilities`** — arrays of account rows (excludes `payslip` bucket accounts). Each row includes **`financialAccountId`**, **`institution`**, **`accountMask`**, **`type`**, **`currency`**, **`liquidity`** (`liquid` \| `semi_liquid` \| `restricted` \| null — from **`financial_account.liquidity`**, F-1), **`side`** (`asset` \| `liability`), **`balance`**, **`balanceAsOf`**, **`balanceSource`** (`manual` \| `import` \| null), **`importFileId`** (when import-derived). Account type classification: **assets** = `checking`, `savings`, `investment`, `retirement`, `health`, `education`; **liabilities** = `credit_card`, `loan`. Accounts with other types (e.g. `payslip`) are excluded. **Liability balances** are stored and displayed as **positive magnitudes** (what you owe); net worth = assets − liabilities.
- **`properties`** — array of real-estate rows for the household. Each entry includes **`propertyId`**, address fields, **`propertyUse`**, latest **`marketValue`** / **`marketValueAsOf`** (from **`property_value_snapshot`** with `as_of_date ≤ asOf`), and optional linked mortgage **`linkedMortgageAccountId`**, **`linkedMortgageBalance`**, **`linkedMortgageAsOf`** when a **`financial_account`** with `type = loan` references the property via **`property_id`**. Property market values are **included in `totals.assets`** (and therefore net worth) when a value exists.
- **`totals`** — **`assets`**, **`liabilities`**, **`netWorth`** (sums where balances exist; `null` when nothing to sum).
- **`memberSummary`** — array of per-person subtotals when the household has **≥ 2** `person_profile` rows; otherwise **`[]`**. Each entry: **`personProfileId`**, **`name`**, **`totalAssets`**, **`totalLiabilities`**, **`netWorth`**. Computed at full household scope (ignores `ownerScope` / `ownerPersonProfileId` on the request). Only accounts with non-null **`owner_person_profile_id`** are attributed to a member; unowned accounts contribute to **`totals`** only (no “Shared” row).
