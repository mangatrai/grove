...
1) Summarize the current state in 8-12 bullets. Include: migrations **`0008`**–**`0010`**; **`/categories`** + **`/categories/rules`**; DB **`category_rule`** + **`classification_meta`**; **`/transactions`** command center (**CR-013**): **All \| Needs review**, **`GET /transactions?needsReview=true`** + **`reviewReasons`**, substring **`search`**, amount filters, **`POST /transactions`** manual entry; **`ResolutionQueuePage`** still primary for bulk status / duplicate-transfer flows until **Story 11.5** (**DOC-005**); **transfer matcher** + **`TRANSFER_*`** env; **cash-summary** + **Home** scope + savings target; **`/settings`** (Household wired). Open work: **11.5** port review queue → **Needs review**, **Epic 6** inbox drill-down, **5.2** matcher coverage, **7** polish, **D-014**, ranked **FTS** (ledger search is substring today).
