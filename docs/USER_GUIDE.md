...
Two sections, since staff are paid weekly and pay periods don't line up with calendar months:
- **This period** — Earned, Paid, and Balance due for the From/To range above it (defaults to current month to date). Because a pay period can straddle the month boundary (work done in one month, paid a few days into the next), this window can show a misleading balance right after a month starts — a payment for last month's work with no matching "earned" in the window, or vice versa.
- **All time** — Total earned, total paid, and running balance due since the staff member's employment start date. Not affected by the date filter; use this for the real, cumulative picture.
...
- Payment-due, deadline, and urgent items also show up inline in your next digest email under "From Your Inbox" — you don't have to open the app to catch them (GH #250). Lower-priority items are summarized as a count so the digest stays scannable.
...
- **My Pay** — under **This period**, pick a date range (defaults to the current month) to see what you've **earned** (approved timesheet hours at the rate in effect when worked, plus approved expenses), what's been **paid** (transactions your household's owner/admin has tagged to you under Salary/Bonus/Reimbursement), and the resulting **balance due for that period**. Bonuses only appear on the **paid** side, once tagged as a transaction — there's no separate bonus ledger. Because pay periods are weekly and don't line up with calendar months, a period's balance can look off right after a month starts (a payment for last month's work showing up with no matching earned amount in the window, or vice versa) — below it, an **All time** section shows the same three figures since your employment start date, unaffected by the date range, for the real cumulative picture. **Download hours report** and **Download payment report** buttons export a PDF for the selected date range — the hours report lists every logged day with its status (draft/submitted/approved/rejected); the payment report mirrors the earned/paid/balance breakdown shown on screen.
