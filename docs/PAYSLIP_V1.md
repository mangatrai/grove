...
- **Bank deposit match (CR-068; F-5b + F-5c):** `GET /payslips/:id` returns **`confirmedDeposits`** (stored links; always an array) and **`suggestedDeposits`** (same candidate shape as before; **only non-empty when `confirmedDeposits` is empty**). Suggestions use posted `credit` rows within ±7/±10 calendar days of the pay anchor and 1% (min $0.50) of `net_pay_current`, with `dateDelta` / `amountDelta` on each row. **`PUT` / `DELETE /payslips/:id/deposits/:canonicalId`** add or remove a stored link (idempotent add; delete no-op if missing). Salary-deposit account restriction unchanged when configured on `person_profile`. Detail UI still expects deposit rows until updated in a later slice.
